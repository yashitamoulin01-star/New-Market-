...
export function ImageUploadInput({ name, label, optional, hint, lang = "en" }: Props) {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setUploadError(hi ? "अपलोड विफल" : msg)
    } finally {
      setUploading(false)
    }
  }
  ...
  return (
    <div>
      ...
      {/* Mode toggle */}
      <div className="mb-2 flex rounded-lg border bg-muted/30 p-0.5 w-fit">
        <button
          type="button"
      ...
      {/* Hidden field always carries the final URL */}
      ...
      {mode === "link" ? (
        ...
        <div>
          {url ? (
            ...
            <label className={`flex cursor-pointer items-center gap-2 rounded-md border border-dashed bg-background px-4 py-3 text-sm transition hover:border-primary/50 hover:bg-primary/5 ${uploading ? "opacity-50 pointer-events-none" : ""}`}>
              <Upload size={15} className="text-muted-foreground" />
              <span className="text-muted-foreground">
                {uploading
                  ? (hi ? "अपलोड हो रहा है…" : "Uploading…")
                  : (hi ? "डिवाइस से फ़ोटो चुनें" : "Choose photo from device")}
              </span>
              <input
                ref={fileRef}
                type="file"
